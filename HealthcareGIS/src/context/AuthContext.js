...
function authReducer(state, action) {
  switch (action.type) {
    ...
    case 'LOGIN_SUCCESS':
      return { isAuthenticated: true, user: action.payload, isLoading: false, error: null };
    case 'AUTH_ERROR':
...
export function AuthProvider({ children }) {
  ...
  const loadProfile = useCallback(async (userId) => {
    ...
    if (data) {
      dispatch({ type: 'LOGIN_SUCCESS', payload: mapProfile(data) });
    } else {
  ...
  const signup = useCallback(async (profileData) => {
    ...
    const { data, error } = await supabase.auth.signUp({
      ...
      options: {
        data: { name: profileData.name, role: 'patient' },
      },
    });
    if (error) {
      dispatch({ type: 'AUTH_ERROR', payload: error.message });
